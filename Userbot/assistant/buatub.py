...
from Userbot.helper.loyalty import (
    add_loyalty_point, get_loyalty_point, reset_loyalty_point, set_loyalty_point,
    generate_referral_code, save_referral_code, can_generate_new_code
)
...
async def bikin_ubot(client, m, _):
    ...
    if not user_id == kn_client.me.id:
        ...
        return await bot_msg.edit(
            f"<i><b>Gunakan akun anda sendiri, bukan orang lain!!</b></i>"
        )
...
📢 <b><i>CATATAN!</i>
Ada prangkat yang masuk ke akun [UserbotLitex], Jangan dikeluarkan itu userbotnya!.</b>
...
<b>Ads: {Ads()}</b>"""
    ...
    notif_msg = f"\n\n🎁 <b>Loyalty Point</b>: Anda mendapat 1 point!\nTotal: <b>{points}</b>/{min_claim}"
    reward_msg = ""
    if points >= min_claim:
        exp = dB.get_expired_date(kn_client.me.id)
        ...
        if exp is not None:
            ...
            new_exp = exp + timedelta(days=30 * reward_avail)
            dB.set_expired_date(kn_client.me.id, new_exp)
            reward_msg = (
                f"\n\n🎉 <b>Selamat!</b> Anda mendapat bonus <b>{reward_avail} bulan akses</b> "
                f"karena sudah mengumpulkan {min_claim * reward_avail} loyalty point. "
                f"Sisa point sekarang: <b>{sisa}</b>."
            )
            set_loyalty_point(user_id, sisa)
        else:
            set_loyalty_point(user_id, points % min_claim)
            reward_msg = (
                f"\n\n🎉 <b>Selamat!</b> Anda mendapat bonus <b>{reward_avail} bulan akses</b> "
                f"karena sudah mengumpulkan {min_claim * reward_avail} loyalty point. "
                f"Sisa point sekarang: <b>{points % min_claim}</b>."
            )
    await bot.send_message(
        user_id,
        f"<b>Userbot berhasil dibuat!</b>{notif_msg}{reward_msg}"
    )
    # Auto broadcast ke semua client (kecuali user yang baru membuat userbot)
    notif_broadcast = f"🎁 <b>Loyalty Point</b>: {user_id} baru saja mendapatkan 1 point!\nTotal: <b>{points}</b>/{min_claim}"
    await broadcast_to_clients(bot, notif_broadcast, exclude_id=user_id)
    # Generate referral code (IF eligible)
    referral_code = None
    if can_generate_new_code(user_id):
        referral_code = generate_referral_code(user_id, owner_id)
        save_referral_code(user_id, referral_code)
        # Notif + tombol claim/skip
        ref_msg = (
        ...
        await bot.send_message(
            user_id, ref_msg, reply_markup=ref_buttons
        )
        # Broadcast ke semua client
        bc_msg = (
            f"📣 User <a href='tg://user?id={user_id}'>{m.from_user.first_name}</a>\nBaru saja mendapat kode referral: <code>{referral_code}</code>. "
            "Ajak temanmu, dapatkan loyalty point ekstra!"
        )
        await broadcast_to_clients(bot, bc_msg, exclude_id=user_id)
    else:
        # Sudah ada referral aktif, infokan
        active = mongo_db["referrals"].find_one({"user_id": user_id, "used": False})
        await bot.send_message(
            user_id,
            f"Kamu sudah punya kode referral aktif: <code>{active['code']}</code>",
            reply_markup=Button.referral_menu(active['code'])
        )
    # Ambil ulang point dan total claim setelah proses loyalty
    current_points = get_loyalty_point(user_id)
